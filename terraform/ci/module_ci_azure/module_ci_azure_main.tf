resource "azurerm_resource_group" "ci_group" {
  name = var.group_name
...
resource "azurerm_virtual_network" "vnet_ci" {
  ...
  location            = var.group_location
  depends_on          = [azurerm_resource_group.ci_group]
}

resource "azurerm_subnet" "subnet_ci" {
...
resource "azurerm_network_interface" "nic_ci_nodes" {
  ...
  ip_configuration {
    ...
    subnet_id                     = azurerm_subnet.subnet_ci.id
  ...
  depends_on = [azurerm_resource_group.ci_group, azurerm_public_ip.pubip_node_web]
}
...
resource "azurerm_virtual_network" "vnet_ci_controller" {
...
resource "azurerm_subnet" "subnet_ci_controller" {
  name                 = "subnet_controller_${var.environment}_internal"
  resource_group_name  = var.group_name
  virtual_network_name = azurerm_virtual_network.vnet_ci_controller.name
  address_prefixes     = var.env_subnet_space_cidr_control
  depends_on           = [azurerm_resource_group.ci_group]
}

resource "azurerm_network_interface" "nic_ci_controller" {
  name                = "nic_${var.environment}_controller"
  resource_group_name = var.group_name
  location            = var.group_location
  ip_configuration {
    name                          = "nic_controller_config"
    subnet_id                     = azurerm_subnet.subnet_ci_controller.id
...
resource "azurerm_linux_virtual_machine" "nodecontroller" {
  ...
  network_interface_ids = [
    azurerm_network_interface.nic_ci_controller.id
  ]
...
resource "azurerm_linux_virtual_machine" "nodes" {
  ...
  disable_password_authentication = true
  network_interface_ids = [
    azurerm_network_interface.nic_ci_nodes[count.index].id
  ]
...
#########################
resource "azurerm_network_security_group" "ci_security_group_nodes" {
...
# Association
resource "azurerm_network_interface_security_group_association" "ci_sga_nodes" {
  for_each = { for idx, nic in azurerm_network_interface.nic_ci_nodes : idx => nic }

  network_interface_id      = each.value.id
  network_security_group_id = azurerm_network_security_group.ci_security_group_nodes.id
  depends_on                = [azurerm_network_interface.nic_ci_nodes, azurerm_network_security_group.ci_security_group_nodes]
}
...
resource "azurerm_network_security_group" "ci_security_group_controller" {
  ...
  security_rule {
    ...
    destination_port_range     = "22"
    source_address_prefix      = "*"
    destination_address_prefix = azurerm_subnet.subnet_ci_controller.address_prefixes[0]
  }
...
# Association
resource "azurerm_network_interface_security_group_association" "ci_sga_controller" {
  network_interface_id      = azurerm_network_interface.nic_ci_controller.id
  network_security_group_id = azurerm_network_security_group.ci_security_group_controller.id
  depends_on                = [azurerm_network_security_group.ci_security_group_controller, azurerm_network_interface.nic_ci_controller]
}
...
# Peering connection from vnet_ci_controller to vnet_ci
resource "azurerm_virtual_network_peering" "vnet_controller_to_ci" {
  name                         = "vnet-controller-to-prod"
  resource_group_name          = var.group_name
  virtual_network_name         = azurerm_virtual_network.vnet_ci_controller.name
  remote_virtual_network_id    = azurerm_virtual_network.vnet_ci.id
  allow_virtual_network_access = true
  allow_forwarded_traffic      = false
  allow_gateway_transit        = false
  use_remote_gateways          = false

  depends_on = [azurerm_resource_group.ci_group, azurerm_virtual_network.vnet_ci, azurerm_virtual_network.vnet_ci_controller]
}

# Peering connection from vnet_ci to vnet_ci_controller
resource "azurerm_virtual_network_peering" "vnet_ci_to_controller" {
  name                         = "vnet-prod-to-controller"
  resource_group_name          = var.group_name
  virtual_network_name         = azurerm_virtual_network.vnet_ci.name
  remote_virtual_network_id    = azurerm_virtual_network.vnet_ci_controller.id
  allow_virtual_network_access = true
  allow_forwarded_traffic      = false
  allow_gateway_transit        = false
  use_remote_gateways          = false
